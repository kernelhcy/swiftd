...
\section{动态链接技术}
	...
	在Linux下和Window下，动态库的创建和使用是不相同的。由于本课题主要是在\#Linux系统中进行，因此在这里，将通过一个例子来简单的介绍在Linux下动态库的使用\ucite{cxyzwxy}。例子的程序源码如下：
	\begin{lstlisting}
/* Program1.c*/
#include "Lib.h"
...
#define PLUGIN_CALL_HANDLER(x,y)\
 	...
 	{\
 		if (NULL == srv || NULL == con){return HANDLER_ERROR;}\
  		if (!srv -> slots -> ptr){return HANDLER_GO_ON;}\
  		if (!srv -> slots -> ptr[x]){return HANDLER_GO_ON;}\
   		pthread_mutex_lock(&srv -> plugin_lock);\
   		plugin *p;size_t i;handler_t ht;\
...
{
	handler_t ht;
	/* 在此处，函数对资源地址进行解析。存放解析好的资源地址。*/
	//得到没有解码的资源地址。调用插件。
	switch(ht = plugin_handle_url_raw(srv, con)){
		case HANDLER_GO_ON:break;
		case HANDLER_FINISHED:return ht;
		default:break;
	}
	//解码资源地址和查询数据。
	//解码资源地址。调用插件。
